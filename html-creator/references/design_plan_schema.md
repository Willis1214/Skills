...
4. Every analytical component must name the data or relationship that justifies it.
5. `canvas` keeps one dominant claim and one primary proof object.
6. Missing or weak evidence is shown as `assumption`, `TBD`, or `unknown`; it is never filled by decorative numbers.
7. The plan may be kept as a temporary implementation note, but the final HTML must remain self-contained.
