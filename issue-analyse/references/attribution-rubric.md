...
3. 破局策略: 当前是需要冲突扩大、底层溯源、归因防御，还是困境转化？
4. 合同 / 约定: 哪个明确合同、边界或验收标准适用？
5. 履约情况: 谁遵守或没有遵守该约定？
6. 因果关系: 哪个事件或缺失输入实质造成了影响？
7. 证据: 哪些事实支持或削弱该归因？
8. 结论把握度: 结论强度有多高？
...
| 策略 | 可采用条件 | 必要证据 / 上下文 | 不能这么用 |
| --- | --- | --- | --- |
| `冲突扩大法` | 单点 issue 被用来推出局部责任，但按该逻辑应影响更多系统范围。 | 系统架构、相关 project、历史 / 当前 release、上下游依赖、对方判断逻辑原文。 | 不能编造牵连范围；不能把“没人投诉”当成唯一证明。 |
| `底层溯源法` | 争议卡在表层现象或他人判错基准。 | spec、SOP、contract、baseline、acceptance criteria、系统 invariant。 | 不能无限追问；下一层必须有可验证依据。 |
| `归因防御策略` | 成熟稳定系统突然被归因，且存在外部新变量可能。 | 我方稳定性证据、最近变更记录、监控/回归、上游输入、环境/配置/release 变化。 | 不能用稳定历史替代证据；不能无脑甩锅。 |
| `困境转化法` | corner / edge / special case 超出单点处理边界。 | 触发条件、Owner/RACI、现有合同覆盖范围、修复和预防 gate。 | 不能把普通本地 bug 包装成系统问题。 |

破局策略只决定调查和沟通路径，不直接决定责任归属。责任仍必须回到合同、履约、因果和证据链。
