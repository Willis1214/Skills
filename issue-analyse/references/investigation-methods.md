...
- 排查陷入僵局，继续单人低层 debugging 只会扩大时间成本。
- 争议已经超出单点执行动作，需要把问题重新放回系统、合同、上下游和流程边界中判断。

默认顺序:
1. 先质疑问题是否成立: 不把 complaint、alarm、review comment 直接等同于 defect。
2. 再还原系统上下文: 当前、历史、相关 project、上下游、合同、baseline。
3. 再选择破局策略: `冲突扩大法`、`底层溯源法`、`归因防御策略`、`困境转化法`。
4. 再决定下一步: 本地修复、补证据、反问基准、拉通 review、流程修补或沟通口径。

工作流改进:
- 面对他人提出 issue 时，第一动作不是接 bug，而是提出最小证据问题。
- 面对复杂僵局时，停止围绕报错本身打转，改查“判断该报错的基准是否有效”。
- 面对成熟系统突发异常时，把新输入、新上游、新环境、新发布控制作为一线假设，同时列出我方稳定性证据。
- 面对 corner / edge / special case 时，不把全部压力留在一个执行者身上，而是拆成 root-cause owner、fix owner、prevention owner。

## 冲突扩大法

使用场景:
- 对方基于一个 single issue 推导出“这里一定是你的错”。
- 用户陷入局部自证，只能解释当前点，无法检验对方逻辑本身是否成立。
- 同类系统、复制来源、兄弟 project、上下游链路按对方逻辑应当一起受影响。

核心动作:
- 先暂时接受对方前提: “如果这个判断逻辑成立...”
- 沿系统边界推演: 哪些模块、release、客户、baseline、上游输入、下游输出也应当出现同类异常？
- 反问缺口: 为什么这些地方没有出问题、没有报警、没有投诉，或历史上已经被接受？
- 把讨论从“单点背锅”拉回“系统性一致性检验”。

待补证据:
- 系统架构、模块依赖、复制来源、相关 project、历史 release、当前 release、上下游接口记录。
- 对方判断逻辑的明确表述，而不是转述后的情绪化结论。

可能结论:
- 对方逻辑只能解释单点观察，不能解释系统事实，问题尚未成立。
- 如果对方逻辑成立，应当触发更大范围 review，不能只由单一 Owner 承担。
...
稳妥表达:
- “如果按这个判断逻辑成立，同一 baseline / 同一上游输入 / 同类 project 理论上也应该出现相同影响。现在需要先对比这些位置，否则不能只基于单点观察归因。”
...
稳妥表达:
- “现在不先讨论谁负责，先确认判定这个现象为 issue 的底层基准是什么；如果基准不适用，归因结论也不能成立。”
...
稳妥表达:
- “这个系统长期稳定只能说明我方底座不是默认第一嫌疑，但还需要用最近变更、监控和输入日志证明。当前应优先核对新增上游/环境变量，而不是直接把问题归到本模块。”
...
- 输出 win-win 路径: 先止血、再修 contract/gate、最后沉淀防复发规则。
...
稳妥表达:
- “这个 case 已经超出单点修复边界，更像系统演进带来的新边界问题。建议把修复 Owner、根因 Owner 和防复发 Owner 拆开，拉上下游做一次最小 review。”
